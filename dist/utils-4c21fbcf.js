...
const Pe = "-", zr = (r) => {
  ...
  return {
    getClassGroupId: (d) => {
      const i = d.split(Pe);
      return i[0] === "" && i.length !== 1 && i.shift(), Ze(i, o) || Ir(d);
    },
  ...
  if (qe.test(r)) {
    const o = qe.exec(r)[1], s = o == null ? void 0 : o.substring(0, o.indexOf(":"));
    if (s)
      return "arbitrary.." + s;
  }
}, Mr = (r) => {
  ...
  return Wr(Object.entries(r.classGroups), s).forEach(([l, d]) => {
    Te(d, n, l, o);
  }), n;
}, Te = (r, o, s, n) => {
  r.forEach((c) => {
...
}, Fr = (r) => r.isThemeGetter, Wr = (r, o) => o ? r.map(([s, n]) => {
  ...
  for (let p = d.length - 1; p >= 0; p -= 1) {
    ...
    const E = Lr(v).join(":"), C = _ ? E + Qe : E, h = C + P;
    if (l.includes(h))
      continue;
    l.push(h);
    const Y = c(P, z);
...
function Ur(r, ...o) {
  ...
  function d(p) {
    ...
    return s = Nr(R), n = s.cache.get, c = s.cache.set, l = i, i(p);
  }
  function i(p) {
    const R = n(p);
    if (R)
...
const w = (r) => {
...
}, rr = /^\[(?:([a-z-]+):)?(.+)\]$/i, Br = /^\d+\/\d+$/, Jr = /* @__PURE__ */ new Set(["px", "full", "screen"]), qr = /^(\d+(\.\d+)?)?(xs|sm|md|lg|xl)$/, Kr = /\d+(%|px|r?em|[sdl]?v([hwib]|min|max)|pt|pc|in|cm|mm|cap|ch|ex|r?lh|cq(w|h|i|b|min|max))|\b(calc|min|max|clamp)\(.+\)|^0$/, Xr = /^(rgba?|hsla?|hwb|(ok)?(lab|lch))\(.+\)$/, Hr = /^(inset_)?-?((\d+)?\.?(\d+)[a-z]+|0)_-?((\d+)?\.?(\d+)[a-z]+|0)/, Zr = /^(url|image|image-set|cross-fade|element|(repeating-)?(linear|radial|conic)-gradient)\(.+\)$/, $ = (r) => Z(r) || Jr.has(r) || Br.test(r), D = (r) => Q(r, "length", st), Z = (r) => !!r && !Number.isNaN(Number(r)), ke = (r) => Q(r, "number", Z), le = (r) => !!r && Number.isInteger(Number(r)), Qr = (r) => r.endsWith("%") && Z(r.slice(0, -1)), f = (r) => rr.test(r), V = (r) => qr.test(r), et = /* @__PURE__ */ new Set(["length", "size", "percentage"]), rt = (r) => Q(r, et, tr), tt = (r) => Q(r, "position", tr), ot = /* @__PURE__ */ new Set(["image", "url"]), nt = (r) => Q(r, ot, lt), at = (r) => Q(r, "", it), ce = () => !0, Q = (r, o, s) => {
  const n = rr.exec(r);
  ...
  Kr.test(r) && !Xr.test(r)
), tr = () => !1, it = (r) => Hr.test(r), lt = (r) => Zr.test(r), ct = () => {
  ...
  return {
    ...
    theme: {
      ...
      gradientColorStopPositions: [Qr, D],
      inset: re(),
      margin: re(),
      opacity: F(),
      padding: y(),
